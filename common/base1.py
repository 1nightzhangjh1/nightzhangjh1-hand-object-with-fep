...
from common.nets.loss import CoordLoss, NormalVectorLoss, EdgeLengthLoss
...
class Trainer(Base):
    ...
    self.mano = MANO()
    self.face = self.mano.layer.th_faces.cpu().numpy()

   
    self.coord_loss = CoordLoss()
    self.normal_loss = NormalVectorLoss(self.face)
    self.edge_loss = EdgeLengthLoss(self.face)

    def compute_loss(self, outputs, targets):
        hand_pred = outputs['hand_pred']  # [B, 778, 3]
        joint_pred = outputs['joint_pred']  # [B, 21, 3]
        mesh_gt = targets['fit_mesh_cam']  # [B, 778, 3]
        joint_gt = targets['fit_joint_cam']  # [B, 21, 3]

        loss_coord = self.coord_loss(hand_pred, mesh_gt, valid=1).mean()
        loss_joint = self.coord_loss(joint_pred, joint_gt, valid=1).mean()
        loss_edge = self.edge_loss(hand_pred, mesh_gt, valid=1).mean()
        loss_normal = self.normal_loss(hand_pred, mesh_gt, valid=1).mean()

        total_loss = loss_coord + loss_joint + loss_edge + loss_normal
